...
export default function ProfileBuilderPage({ params }: { params: { id: string } }) {
  const {
    profile, schema, loading, saving, mappings, getSourceColumns, updatePairing,
    totalUnresolved, totalCols, markReady, downgradeToDraft, saveDraft
  } = useProfileBuilder(params.id)
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="border-b p-4 flex items-center justify-between">
        ...
        <button onClick={saveDraft} disabled={saving} className="px-4 py-2 border rounded hover:bg-gray-50">
          Simpan Draft
        </button>
      </header>
      ...
      <ConfirmDialog
        ...
        showCancel={profile.status === 'ready' && dialogMessage.startsWith('Mengubah')}
        onConfirm={downgradeToDraft}
