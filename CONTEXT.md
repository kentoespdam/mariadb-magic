...
- **Session Confirmation (Issue mariadb-magic-62m)**: Implementasi UI Step 2 (Konfirmasi) di `/sessions/new`. Routing menggunakan query parameter `?profile={id}` untuk menampilkan screen konfirmasi sebelum men-trigger `sessionService.start(profileId)` dan melakukan redirect ke Dashboard (`/`).
