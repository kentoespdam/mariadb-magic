...
## Corpus Check
- 50 files · ~47,080 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 394 nodes · 609 edges · 27 communities detected
- Extraction: 72% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 173 edges (avg confidence: 0.8)
...
1. `run()` - 22 edges
2. `handleAPI()` - 15 edges
3. `Close()` - 13 edges
4. `ProfilesHandler` - 12 edges
...
Cohesion: 0.24
Nodes (7): ConnectionHandler, getID(), CreateConnectionRequest, getProfileID(), ProfilesHandler, handleAPI(), NewIntrospector()

### Community 4 - "Community 4"
Cohesion: 0.11
Nodes (14): PassphraseProvider, NewBootstrapper(), Bootstrapper, EnsureDB(), HasDB(), Heal(), quarantineAndRebuild(), extractVersion() (+6 more)
...
Cohesion: 0.13
Nodes (9): NewProfilesHandler(), NewMappingProfilesRepo(), scanMappingProfileRows(), strPtr(), TestValidateProfileForReady(), ValidateProfileForReady(), MappingProfilesRepo, ValidationError (+1 more)
...
Cohesion: 0.23
Nodes (5): NewClosureAdvisor(), TestClosureAdvisorExpand(), TestClosureAdvisorTopologicalSort(), ClosureAdvisor, TableWithRole
...
## Knowledge Gaps
- **96 isolated node(s):** `TableWithRole`, `Params`, `KeyMode`, `KeyProvider`, `ProviderFactory` (+91 more)
...
- **Why does `run()` connect `Community 1` to `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 8`, `Community 11`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `Close()` connect `Community 8` to `Community 1`, `Community 3`, `Community 4`, `Community 6`, `Community 7`?**
...
- **Are the 13 inferred relationships involving `handleAPI()` (e.g. with `.List()` and `.Create()`) actually correct?**
  _`handleAPI()` has 13 INFERRED edges - model-reasoned connections that need verification._
- **Are the 12 inferred relationships involving `Close()` (e.g. with `.List()` and `.List()`) actually correct?**
  _`Close()` has 12 INFERRED edges - model-reasoned connections that need verification._
