...
export function useProfileBuilder(profileId: string) {
  ...
  const getSourceColumns = useCallback(() => {
  ...
  const buildInitialMappings = useCallback((): ProfileMappings => {
    ...
    return {
      tables: schema.tables.map(t => {
        ...
        return { table_name: t.name, column_pairs: cols, unresolved_cnt: unresolved, total_cols: cols.length }
      })
    }
  }, [schema, autoMatch])
  ...
  useEffect(() => {
    if (schema && mappings.tables.length === 0) {
      ...
      if (profile?.column_pairings_json) {
        try {
          setMappings(JSON.parse(profile.column_pairings_json))
        } catch { setMappings(initial) }
  ...
  return {
    profile, schema, loading, saving, mappings, error, getSourceColumns, updatePairing,
    totalUnresolved, totalCols, markReady, downgradeToDraft, saveDraft, setError
  }
}