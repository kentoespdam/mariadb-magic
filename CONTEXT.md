...
- **Session Runner Context & Cancellation (Issue mariadb-magic-jm8)**: Fix bug dimana context request HTTP terbawa ke background goroutine sync runner (menyebabkan session berhenti prematur). Implementasi `context.WithoutCancel` untuk detach dan penyimpanan `cancelFunc` di `Runner.cancels` map untuk mendukung pembatalan manual via `/api/sessions/{id}/cancel`.
- **Session Log API Shape Fix**: `/api/sessions/{id}/logs` sekarang mengembalikan object `{items: [], next_cursor: string}` sesuai ekspektasi frontend, bukan lagi flat array.
- **Cleanup Redundant Handlers**: Menghapus `internal/api/sessions.go` yang duplikat dan mengonsolidasikan semua logic sesi ke dalam `ProfilesHandler` demi konsistensi routing di `main.go`.
